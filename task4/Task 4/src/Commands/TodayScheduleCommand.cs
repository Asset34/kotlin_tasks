...
namespace Task_4
{
    public class TodayScheduleCommand(TelegramBotClient botClient) : ICommand
    {
        public async Task ExecuteAsync(List<string> args, Update update, CancellationToken cancelToken)
        {
            ...
            // Action
            ScheduleDay? scheduleDay = await LETIScheduleReceiver.ReceiveTodayScheduleAsync(group);
